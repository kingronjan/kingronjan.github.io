...
上面的写法支持 SQLite 和 PostgreSQL，在 MySQL 8.0 中，需要略微改动下，在每个 value 组前面加上 `row` 关键字，如下：

```sql
sqlite> WITH countries AS (
   ...>   SELECT * FROM (VALUES
...
参考：[MySQL :: MySQL 8.0 Reference Manual :: 15.2.19 VALUES Statement](https://dev.mysql.com/doc/refman/8.4/en/values.html "MySQL :: MySQL 8.0 Reference Manual :: 15.2.19 VALUES Statement")
