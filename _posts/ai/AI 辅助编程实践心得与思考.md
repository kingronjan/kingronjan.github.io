...
categories:
- ai
date: 2026-01-15 17:49 +0800
layout: post
tags:
- ai
title: AI 辅助编程实践心得与思考
---

AI 编码工具正以前所未有的方式改变着软件开发。在这篇文章中，我将分享一些使用 AI 编码工具的心得体会，涵盖从开发模式、提效技巧到核心理念的思考，希望能为你提供一些有价值的参考。
...
建议在使用合适的工具前，可以完整阅读下相关的官方文档，一般都会有比较完善的使用案例和技巧。
...
对于一些小工具，比较偏向于使用这种方式，把 AI 写的代码当作一个黑盒，从不打开看，让 AI 自己去编写、验证、修改，我看着哪里不好，就给出修改意见，哪怕这个意见比较模糊，最后直到我满意为止。这里可以参考 Andrej Karpathy 在推特上创造 “vibe coding”（氛围编码）一词时提到的内容：
...
- **拆分需求**：将大型需求拆解成小而明确的任务，避免让 AI 一次性处理过多内容，这能有效减少“幻觉”和上下文过长的问题
- **迭代提示词**：在与 AI 的初期沟通中，将反复确认的内容总结提炼，用于改进和优化生成提案的提示词
...
除了开发模式，一些高级技巧能进一步释放 AI 的生产力。



## BMAD-METHOD

BMAD-METHOD 的核心是让 AI 扮演产品经理、架构师、开发、测试等多种角色，组建一个虚拟的专业团队。各种 AI 可能有其自己的实现方式，当然也有开源的实现，可以适配多种 AI 工具：
...
这是一种值得学习和实践的方式，我在实际使用中还没怎么用到，但以后一定会找机会尝试。除此之外，我觉得这也适用于与 CI/CD 流程结合，引入安全评审、代码规范检查等自动化角色。
...
测试可以让 AI 快速的验证自己写的代码有无问题，如果你在写开始项目之前已经有非常完善的测试，那么完全可以放心的让 AI 遵循测试驱动开发的流程来工作，只要让 AI 记住，不要修改测试文件，如果它认为确实有必要修改，也需先征得你的同意，并给出充分的理由。

另一方面，也可以为 AI 提供完备的测试规则，让它根据规则生成测试代码，然后验证，改进业务代码。这方面可以参考示例：[whenwords/tests.yaml](https://github.com/dbreunig/whenwords/blob/main/tests.yaml)。



## 提示词：与 AI 高效沟通的关键

高质量的提示词是与 AI 高效协作的核心。网络上有许多优秀的资源可供借鉴，例如 iFlow 社区的全局提示词分享、Cursor 的 MDC 规则等。这些资源即使平台不同，其背后的思想和结构也极具参考价值。

我发现的一些值得参考的内容：

- [分享个人在用的 IFLOW 编程全局提示词](https://vibex.iflow.cn/t/topic/257) 适用于 iflow，同时也可以改进以适配 gemini，claude 等
- [awesome-cursor-rules-mdc/rules-mdc/python.mdc](https://github.com/sanjeed5/awesome-cursor-rules-mdc/blob/main/rules-mdc/python.mdc) 适用于 cursor，但对于其他 AI 工具仍有参考价值
